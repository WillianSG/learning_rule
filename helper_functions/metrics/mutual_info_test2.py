...
S = Synapses(G, G, 'w : 1', on_pre='v_post += 1.0*w')
...
for x in range(0, len(spikemon.i)):
	if spikemon.i[x] == 0:
		...
		out_neuron_spkts.append(np.round(spikemon.t[x]/second, 3))

neuron1_spkts = np.array(neuron1_spkts)
neuron2_spkts = np.array(neuron2_spkts)
out_neuron_spkts = np.array(out_neuron_spkts)
